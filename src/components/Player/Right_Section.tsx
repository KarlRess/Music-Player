...
const Right_Section = () => {
  ...
  return (
    <div className="flex h-full w-[19%] items-center justify-end gap-2 pr-2">
      {/* Volume Icon */}
      <div
      ...
      {/* Volume Bar */}
      <div
        className="flex-center group/volume h-5 w-22.5 cursor-pointer active:cursor-grabbing"
        ref={volumeContainerRef}
        onMouseDown={onSeekStartVolume}
      >
        <div className="bg-icon h-1 w-full rounded-full">
          <div
            ...
            ref={volumeBarRef}
          >
            <div className="absolute -right-1.5 h-3 w-3 rounded-full bg-white opacity-0 group-hover/volume:opacity-100 group-active/volume:opacity-100"></div>
          </div>
        </div>
      </div>
    </div>
